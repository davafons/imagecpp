...
BACDialog::BACDialog(Document *document, QWidget *parent)
  ...
  // -- Connect sliders
  connect(&red_bright_slider_, &QSlider::valueChanged, &operation(), [this](int value) {
    operation().setMean(value, BrightnessAndConstrast::Channel::Red);
  });

  connect(
      &green_bright_slider_, &QSlider::valueChanged, &operation(), [this](int value) {
        operation().setMean(value, BrightnessAndConstrast::Channel::Green);
      });

  connect(
      &blue_bright_slider_, &QSlider::valueChanged, &operation(), [this](int value) {
  ...
  // -- Connect sliders
  connect(
      &red_contrast_slider_, &QSlider::valueChanged, &operation(), [this](int value) {
        operation().setStd(value, BrightnessAndConstrast::Channel::Red);
      });

  connect(
      &green_contrast_slider_, &QSlider::valueChanged, &operation(), [this](int value) {
        operation().setStd(value, BrightnessAndConstrast::Channel::Green);
      });

  connect(
      &blue_contrast_slider_, &QSlider::valueChanged, &operation(), [this](int value) {
