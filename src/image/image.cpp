...
// Properties
int Image::width() const noexcept { return image_.width(); }
int Image::height() const noexcept { return image_.height(); }
QSize Image::size() const noexcept { return image_.size(); }
QRect Image::rect() const noexcept { return image_.rect(); }
QImage::Format Image::format() const noexcept { return image_.format(); }
size_t Image::pixelCount() const noexcept {
  return image_.width() * image_.height();
}

// Pixel Operations
QRgb *Image::bits() { return (QRgb *)image_.bits(); }
QRgb *Image::scanLine(int i) { return (QRgb *)image_.scanLine(i); }
const QRgb *Image::constBits() const { return (QRgb *)image_.constBits(); }
const QRgb *Image::constScanLine(int i) const {
  return (QRgb *)image_.constScanLine(i);
}

QRgb Image::pixel(int x, int y) const { return image_.pixel(x, y); }
...
QColor Image::pixelColor(int x, int y) const { return image_.pixelColor(x, y); }
